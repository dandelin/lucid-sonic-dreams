...
if not os.path.exists("stylegan2"):
    pygit2.clone_repository("https://github.com/NVlabs/stylegan2-ada.git", "stylegan2")
...
class LucidSonicDream:
    ...
    def generate_frames(self):
        ...
        Gs_syn_kwargs = {
            "output_transform": {"func": convert_images_to_uint8, "nchw_to_nhwc": True},
        ...
        for i in tqdm(range(num_frame_batches), position=0, leave=True):
            ...
            for j, image in enumerate(image_batch):
                ...
                # Apply efects
                for effect in self.custom_effects:
                    array = effect.apply_effect(array=array, index=(i * batch_size) + j)
                ...
                file_name = str((i * batch_size) + j).zfill(len(str(max_frame_index)))
